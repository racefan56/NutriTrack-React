...
export const updateOtherUser = createAsyncThunk(
  ...
  async ([userId, formData], thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.token;
      return await userService.updateOtherUser(userId, formData, token);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const deleteOtherUser = createAsyncThunk(
  ...
  async (userId, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.token;
      return await userService.deleteOtherUser(userId, token);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteOtherUser.fulfilled, (state) => {
        state.user = null;
        state.loading = false;
        state.isSuccess = true;
      })
